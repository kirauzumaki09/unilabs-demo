...
export default function FundMain() {
  ...
  const currentBlogs = reversedBlogList.slice(indexOfFirstBlog, indexOfLastBlog);
          ...
          {currentBlogs.map((item, index) => (
            ...
            >
              <div className="flex flex-col py-[65px] pl-[55px] max-xl:pl-4 max-xl:py-0 max-md:pl-0">
                ...
                <h2 className="text-white font-anybody font-semibold text-[36px] max-sm:text-[28px] mb-[16px]">
                  {item.title}
                </h2>
                ...
                <Link
                  href={item.link}
                  className="mt-auto text-[18px] font-medium text-[#2DC4EC]"
                >
                  Learn More
                </Link>
              </div>
              <figure className="size-[556px] max-xl:size-[300px] max-md:w-full   max-md:rounded-[20px] flex-shrink-0 rounded-[40px] overflow-hidden relative">
                <Image
                  src={item.image}
                  width={1920}
                  height={556}
